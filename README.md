...
On Windows, cache hooks and CRITIC use the bundled PowerShell/`.cmd` launchers selected by `hooks.json`. The managed remote MCP works normally; `cachelayer_tools/run_mcp.cmd` is bundled for hosts or user MCP configurations that can select a Windows command.
