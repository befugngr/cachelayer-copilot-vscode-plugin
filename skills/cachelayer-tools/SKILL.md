...
- Java TIA never treats aggregate `jacoco.xml` as a per-test map. It prefers Smart Picker, seeded STARTS/Ekstazi, runtime-hardened affectedTest, fresh manifest-labeled per-test JaCoCo exec/XML pairs, then Joern CPG usage/data-flow selection and bounded type/import mapping; full-suite escalation is refused.
...
- STARTS/Ekstazi selection counts prefer official plugin output and use native artifact parsing only as fallback. Deep module artifacts are scanned without a visit cap; set `CACHELAYER_TIA_ARTIFACT_PATHS` for unusual external roots.
