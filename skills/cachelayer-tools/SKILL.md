...
- Java TIA never treats aggregate `jacoco.xml` as a per-test map. It prefers Smart Picker, seeded STARTS/Ekstazi, runtime-hardened affectedTest, native per-test JaCoCo reports, then Joern CPG usage/data-flow selection and bounded type/import mapping; full-suite escalation is refused.
